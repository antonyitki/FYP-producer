...
from preproces_chinaHL import generate_training_sequences, SEQUENCE_LENGTH
...
SAVE_MODEL_PATH = "modelchinaHL.h5"
...
def build_model(output_units, num_units, loss, learning_rate):
    ...
    y = keras.layers.LSTM(num_units[1])(input)
    #y = keras.layers.Dropout(0.2)(y)
    '''
    z = keras.layers.LSTM(num_units[2])(input)
    #z = keras.layers.Dropout(0.2)(z)
    t = keras.layers.LSTM(num_units[2])(input)
    #t = keras.layers.Dropout(0.2)(t)
    k = keras.layers.LSTM(num_units[2])(input)
    #k = keras.layers.Dropout(0.2)(k)
    l = keras.layers.LSTM(num_units[3])(input)
    #l = keras.layers.Dropout(0.2)(l)
    m = keras.layers.LSTM(num_units[4])(input)
    m = keras.layers.Dropout(0.2)(m)
    '''
    
    output = keras.layers.Dense(output_units, activation = "softmax")(x),(y)#,(z),(t),(k),(l),(m)
